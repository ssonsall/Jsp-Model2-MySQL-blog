		...
		<div class="row">
			<div class="col-lg-8">
				<div class="blog_left_sidebar">
					<h1>"${keyword}"에 대한 검색결과 : ${countRow}건</h1>
					<!-- 블로그 글 시작 -->
					<c:choose>
					...
					<nav class="blog-pagination justify-content-center d-flex">
						<ul class="pagination">
							...
							<c:if test="${navPage ne 0}">
								<li class="page-item"><a href="/blog/board?cmd=search&keyword=${param.keyword}&page=${navPage*5}" class="page-link" aria-label="Previous"> <span aria-hidden="true"> <span class="lnr lnr-chevron-left"></span>
									</span>
								</a></li>
							</c:if>
								...
								<c:if test="${i le count}">
									<c:choose>
										<c:when test="${i eq param.page}">
											<li class="page-item active"><a href="/blog/board?cmd=search&keyword=${param.keyword}&page=${i}" class="page-link"> <span aria-hidden="true">${i}</span>
											</a></li>
										</c:when>
										<c:otherwise>
											<li class="page-item"><a href="/blog/board?cmd=search&keyword=${param.keyword}&page=${i}" class="page-link"> <span aria-hidden="true">${i}</span>
											</a></li>
										</c:otherwise>
									</c:choose>
								</c:if>
							...
							<c:if test="${navPage lt totalPage}">
								<li class="page-item"><a href="/blog/board?cmd=search&keyword=${param.keyword}&page=${navPage*5+6}" class="page-link" aria-label="Next"> <span aria-hidden="true"> <span class="lnr lnr-chevron-right"></span>
									</span>
								</a></li>
							</c:if>
						</ul>
...
<script>
	function searchByAll() {
		...
		location.href = '/blog/board?cmd=search&keyword=' + keyword + '&page=1';
	}
</script>
</body>
</html>