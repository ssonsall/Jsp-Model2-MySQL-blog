      ...
      <div class="row">
         ...
         <div class="col-lg-8">
               ...
               <div class="user_details">
                  ...
                  <div class="float-right">
                     <div class="media">
                        ...
                        <div class="d-flex">
                           <img src="http://localhost:8000/blog/userprofile/${view.userProfile}" width="75px"
                              height="75px">
                        </div>
                     </div>
                  </div>
            ...
            <div class="comment-form" style="margin-top: 0px;">
               <h4 style="margin-bottom: 20px">${view.username} 에게 댓글 남기기</h4>
               <form id="comment-submit">
                  <div class="form-group" id="comment-submit">
                     <input type="hidden" name="userId" value="${sessionScope.user.id}" />
                     <input type="hidden" name="boardId" value="${view.id}" />
                     <textarea id="content" style="height: 60px" class="form-control mb-10" rows="2"
                        name="content" placeholder="Messege" onfocus="this.placeholder = ''"
                        onblur="this.placeholder = 'Messege'" required=""></textarea>
                  </div>
                  <button type="button" onclick="commentWrite()" class="primary-btn submit_btn">Post
                     Comment</button>
               </form>
            </div>
            <!-- 댓글 쓰기 끝 -->
                  ...
                  <div class="comment-list" id="comment-id-${comment.id}">
                     ...
                     <div class="single-comment justify-content-between d-flex">
                        <div class="user justify-content-between d-flex">
                           <div class="thumb">
                              <img src="http://localhost:8000/blog/userprofile/${comment.user.userProfile}"
                        ...
                        <div class="reply-btn">
                           <button onclick="commentDelete(${comment.id})"
                              class="btn-reply text-uppercase">삭제</button>
                           <input type="button"
                              onclick="replyListShow(${comment.id},'${comment.user.username}')"
                              class="btn-reply text-uppercase" id="replyshowclosebtn-id-${comment.id}"
                              value="답글" style="outline: none;" />
                        </div>
                     </div>
                  </div>
                  <!-- 댓글 아이템 끝 -->
...
<script>
   ...
   function commentItemForm(id, username, content, createDate, userProfile) {
      ...
      comment_list += "<input type='button' onclick='replyListShow(" + id + "," + "&#39;" + username + "&#39;" +")' class='btn-reply text-uppercase' id='replyshowclosebtn-id-" + id + "' value='답글' style='outline: none;' /></div></div></div>";
